...
class Improved2ControllerBase(object):
    ...
    def drivePathToGoal(self, path, goalOrientation, plannerDrawer):
        ...
        for waypointNumber in range(0, len(path.waypoints)):
            ...
            y.append(waypoint[1])
        for i in range(len(x)):
            rospy.loginfo("Driving to waypoint (%f, %f)", x[i], y[i])
            waypoint1 = [x[i],y[i]]
            try:
                waypoint2 = [x[i+1],y[i+1]]
            except IndexError:
                waypoint2 = [0,0]
            distance,theta,time = self.driveToWaypoint(waypoint1,waypoint2)
            path_distance = path_distance +distance
            path_theta = path_theta + theta
            time_count = time_count + time
            # Handle ^C
